...
$db = $supabase->initializeDatabase('gimme_score', 'id');
$status = 'pending';
$match_type = 'match';
...
$holes = $planObj[0]->score_details->holes;
...
if($shots == null || empty($shots)){
    ...
    if($count == 0){
        ...
        if($planning_gps->current_shot == $count){
            ...
            $planning_gps->current_shot = $planning_gps->current_shot + 1;
            update_the_hole($id, $holes, $plans_db, $clubs_used, $planning_gps);
...
function update_the_hole($id, $holes, $plans_db, $clubs_used, $planning_gps){

	$update_the_hole_data = [
	    'score_details' => ['holes' => $holes],
	    'planning_gps' => $planning_gps
	];
