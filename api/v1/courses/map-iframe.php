...
try {
    ...
    if($plan == 'plan'){
        ...
        if(empty($plans)){
            http_response_code(400);
            echo json_encode(array("msg" => "No Event record found"));
            exit;
        }
...
let current_shot = null;
</script>

<?php if($load_map){ ?>
<script>

    let location_gps = <?= $location_gps ?>;
    let course_data = <?= $course_data ?>;
    console.log(course_data);
    let holeNumber = '<?= $hole ?>';
    let plot_data = <?= $plot_data ?>;
    let hole_status = '<?= $hole_status ?>';
    let popups = '<?= $popups ?>';
    ...
    current_shot = plot_data.length + 1;
    // alert(mode + 'id :' + id);
    if(mode==='plan'){
        let gimmePlan = supabase
        .channel('gimmePlan')
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'gimme_plan_game', filter: 'id=eq.'+id }, payload => {
            console.log('Change received!', payload);
            shotRealTime(payload, mode);
        })
        .subscribe();
    }else if(mode==='match' || mode==='event'){
        // alert('Test');
        let gimmeScores = supabase
        .channel('gimmeScores')
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'gimme_scores', filter: 'id=eq.'+id }, payload => {
            console.log('Change received!', payload);
            // alert('New Change 2')
            shotRealTime(payload, mode);
        })
...
function shotRealTime(payload, mode){
// alert('triggeresssss');
    var hole_gps = payload.new.hole_gps;
    var new_current_shot = hole_gps.current_shot;
    var new_hole = hole_gps.hole_number;
    if(new_hole != holeNumber){
        console.log('Run hole sequence');
        holeNumber = new_hole;
        $.ajax({
            url: 'https://duendedisplay.co.za/gimme/api/v1/courses/reload_map_data.php',
            method: "POST",
            data: JSON.stringify({
                token: token,
                course_id: course_id,
            ...
            success: function(response) {
                ...
                console.log(response);
                location_gps = response.location_gps;
                course_data = response.course_data;  
                holeNumber =  response.holeNumber;
                plot_data = response.plot_data;
                hole_status = response.hole_status;
                popups = response.popups;
                distance_pref = response.distance_pref;
    ...
    } else if(new_hole == holeNumber){
        var shots = '';

        if(mode=='plan'){
            shots = payload.new.game_data.holes[holeNumber - 1].shots;

        }else{
            shots = payload.new.score_details.holes[holeNumber - 1].shots;

        }

        if (shots.length == current_shot) {
            console.log('Run shot sequence');
            $.ajax({
                url: 'https://duendedisplay.co.za/gimme/api/v1/courses/reload_map_data.php',
                method: "POST",
                data: JSON.stringify({
            ...
            console.log('Current shot did not change or mismatched');
        }
    }
}
</script>
