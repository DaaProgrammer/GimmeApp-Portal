...
if($gamemode=='plan'){
    ...
    try {
        ...
        echo json_encode(array("shots" => $plans[0]->game_data->holes[$hole-1]->shots));
    } catch (Exception $e) {
    ...
    try {
        ...
        if($_POST['roundID']!=0 || $_POST['roundID']!='0'){ 
            if($plans[0]->score_details->holes[$hole-1]->status=="incomplete"){
                http_response_code(200);
                echo json_encode(array("shots" => []));
                exit;
            }
            http_response_code(200);
            echo json_encode(array("shots" => $plans[0]->score_details->holes[$hole-1]->shots));
            exit;
        }
        http_response_code(200);
        echo json_encode(array("shots" => $plans[0]->score_details->holes[$hole-1]->shots));
