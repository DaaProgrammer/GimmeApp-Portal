...
// Get event invitations from database
$query = [
  'select' => '*',
...
$eventRow = $event_db->createCustomQuery($query)->getResult()[0];
...
// Check if user with specified email already exists in JSON
foreach ($invitationRow[0]->invitation_details as $invite) {
  if($invite->email == $invitation['email']){
    http_response_code(400);
    echo json_encode(['error' => 'User already invited','email' => $invitation['email']]);
    exit;
  }
}

// Check if max invitations reached
if(count($invitationRow[0]->invitation_details) >= $eventRow->event_max_players){
  http_response_code(400);
  echo json_encode(['error' => 'Maximum number of invitations already sent','Count' => count($invitationRow[0]->invitation_details),'Max' => $eventRow->event_max_players]);
  exit;
}
...
echo json_encode(array("msg" => 'Success','data' => $updateResult[0]));

// Send email to user
$template = $emailSender->generateUserInvitedTemplate($invitation['email'],$invitation['name'],$eventRow->event_name,$eventRow->event_code);
