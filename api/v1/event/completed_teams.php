    ...
    try{
        ...
        try{
            $scores = $query->select('*')
                ...
                if($filterby!='' || $filterby!=null || !empty($filterby)){
                    $filtered_final_hole_shots_count = array_filter($final_hole_shots_count, function($shot) use ($filterby) {
                        return strtolower($shot['team_name']) == strtolower($filterby);
                    });

                    $filtered_final_hole_shots_count = array_values($filtered_final_hole_shots_count);
                    http_response_code(200);
                    echo json_encode(array("msg" => "Success", 'data' => $filtered_final_hole_shots_count));
                    exit;
                } else {
                    echo json_encode(array("msg" => "Success", 'data' => $final_hole_shots_count));
                }
        }
        catch(Exception $e){
