...
foreach ($team as $member) {
    ...
    try{
        ...
        if(!$updateResult){
            http_response_code(400);
            echo json_encode(array("msg" => "Update not successful ".$member['uid']));
            exit;
        }
    }
    catch(Exception $e){
...
try{
    ...
    $eventObject[0]->match_data->teams[intval($team_index)]->status = 'disqualified'; // Status = active || disqualified
